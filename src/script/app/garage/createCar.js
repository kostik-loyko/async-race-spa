...
const createCar = async () => {
  ...
  createCarBtn.addEventListener('click', () => {
    const createCarAPI = async () => {
      ...
      let carNew = await response.json();
      console.log(carNew);
      let roadNew = `<div class="conteiner-car">
      <div class="conteiner-car-btns">
        <button id="${carNew.id}sel" class="car-btn-select btn-main">select</button>
        <button id="${carNew.id}rem" class="car-btn-remove btn-main">remove</button>
        <h3 class="car-title">${carNew.name}</h3>
      </div>
      <div class="conteiner-car-car">
        <button class="car-btn-move">Move</button>
        <button class="car-btn-stop">Back</button>
        <svg class="car-img" id="${carNew.id}" xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24"
          fill="${carNew.color}">
          <path
      ...
      let container = document.querySelector('.conteiner-main');
      container.insertAdjacentHTML('beforeend', roadNew);
